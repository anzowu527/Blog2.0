...
def _folder_candidates(name: str) -> List[str]:
    """
    Likely image homes for shelter pets.
    We first try Sheltopia, then fall back to Dogtopia/Catopia,
    and finally the Zoolotopia/* variants just in case.
    """
    return [
        os.path.join("images", "shelter", str(name)),
        os.path.join("images", "Shelter", str(name)),
    ]

def pet_image_or_placeholder(name: str) -> str:
    """
    Return base64 of the pet's card image.
    Rule within each candidate folder: {lower_case_name}1.webp
    ...
    for folder in _folder_candidates(name):
    ...
    initials = "".join([p[0].upper() for p in str(name).split() if p])[:2] or "P"
    size = (340, 500)
...
def main():
    ...
    <style>
      ...
      @media (max-width: 768px){
        .sheltopia-title{ font-size:30px !important; }
      }
      /* phones (match gallery breakpoint) */
      @media (max-width: 640px){
        .sheltopia-title{ font-size:24px !important; }
      }
    </style>
    """, unsafe_allow_html=True)

    # Title
    st.markdown("<h1 class='sheltopia-title'>🏠 Welcome to Sheltopia</h1>", unsafe_allow_html=True)
    ...
    # Shelter pets (both Dogs & Cats)
    pets_df = df[(df["Platform"] == "shelter") & (df["Species"].isin(["Dog", "Cat"]))]

    pet_stats = (
        pets_df.groupby("Name", dropna=False)
        .agg({
            "Breed": "first",
            "Sex": "first",
            "Species": "first",
    ...
    # Build card data; precompute route to dog/cat page according to species
    pets = []
    for _, row in pet_stats.iterrows():
        name = str(row["Name"]).strip()
        if not name:
            continue

        species = (str(row["Species"]).strip() or "Pet")
        img_b64 = pet_image_or_placeholder(name)
        ...
        pets.append({
            ...
            "image": img_b64,  # guaranteed string
            "route": f"/?page=shelter&shelter={name}",
        })
    ...
    # Each card uses its own precomputed route so we don't need JS to decide species.
    for pet in pets:
        safe_img = pet.get("image") or pet_image_or_placeholder(pet.get("name", "Pet"))
        name = pet.get("name", "Pet")
        route = pet.get("route", "/")

        cards_html += f"""
        <div class="card" onclick="window.open('{route}', '_blank')">
          <img src="data:image/webp;base64,{safe_img}" alt="{name}" />
          <div class="overlay">
            <h2>{name}</h2>
            <p><strong>Species:</strong> {pet.get('species','')}<br/>
            <strong>Breed:</strong> {pet.get('breed','')}<br/>
            <strong>Gender:</strong> {pet.get('gender','')}<br/>
            <strong>Visit(s):</strong> {pet.get('visits',0)}</p>

            <a class="view-button"
               href="javascript:void(0)"
               onclick="event.stopPropagation(); window.open('{route}', '_blank');">
               View Story →
            </a>
          </div>
        </div>
