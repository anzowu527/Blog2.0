...
def main():
    ...
    # Month selector (newest → oldest)
    if "Arrival Date" not in df.columns or "Departure Date" not in df.columns:
        st.error("Missing required columns: 'Arrival Date' and/or 'Departure Date'.")
        st.stop()

    arr = pd.to_datetime(df["Arrival Date"], errors="coerce")
    dep = pd.to_datetime(df["Departure Date"], errors="coerce")
    if arr.notna().any() and dep.notna().any():
        start = arr.min().replace(day=1)
        end = dep.max().replace(day=1)
        month_starts = pd.date_range(start, end, freq="MS")
        month_options = [f"{d.month}/{d.year}" for d in month_starts[::-1]]  # newest first
    else:
        month_options = []

    selected = st.selectbox(
        "📅 Filter by month",
        options=(["Show All"] + month_options) if month_options else ["Show All"],
        index=0
    )

    # Revenue chart (all time or selected month)
    st.altair_chart(
